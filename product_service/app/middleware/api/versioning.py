"""
API Versioning middleware and utilities for Product Service
...
class APIVersioningMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:  # type: ignore[misc]
        ...
        if not self._is_supported_version(version):
            return Response(
                content='{"detail": "Unsupported API version", "supported_versions": ["1.0", "1.1"]}',
    ...
    def _extract_version(self, request: Request) -> APIVersion:
        ...
        if "application/vnd.productservice.v" in accept_header:
            # Format: application/vnd.productservice.v1.1+json
            try:
                version_part = accept_header.split("application/vnd.productservice.v")[
                    1
                ]
...
def get_api_version(request: Request) -> APIVersion:
    ...
    return getattr(request.state, "api_version", APIVersion(1, 0))
...
def supports_inventory_management(request: Request) -> bool:
    """Check if current API version supports inventory management features"""
    version = get_api_version(request)
    return version.major >= 1 and version.minor >= 1
