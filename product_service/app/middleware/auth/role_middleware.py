"""
Role-based authorization middleware for Product Service.
...
try:
    ...
    logger = setup_product_logging("product_service_roles")
except ImportError:
    logger = logging.getLogger("product_service_roles")
...
class RoleAuthorizationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            ...
            user_roles = getattr(request.state, "user_roles", None) or []
...
def setup_role_authorization_middleware(
    app: FastAPI,
    role_requirements: Optional[Dict[str, Union[str, List[str]]]] = None,
    exclude_paths: Optional[List[str]] = None,
) -> None:
    """
    Setup role-based authorization middleware for Product Service.
    ...
    {
        "/api/v1/products": "admin",  # Admin only
        "/api/v1/categories": ["admin", "moderator"],  # Admin or moderator
        "/api/v1/admin": "admin",  # Admin routes
    }
    """
    ...
    if role_requirements is None:
        # Default role requirements for common patterns
        role_requirements = {
            "/api/v1/products": "admin",  # Product management requires admin
            "/api/v1/categories": [
                "admin",
                "moderator",
            ],  # Categories can be managed by admin/moderator
            "/api/v1/inventory": ["admin", "moderator"],  # Inventory management
            "/api/v1/admin": "admin",  # Admin-only routes
        }
