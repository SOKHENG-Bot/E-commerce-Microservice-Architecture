...
logger = setup_logging("user_service.events.kafka", log_level=get_settings().LOG_LEVEL)
...
class KafkaEventPublisher(EventPublisher):
    ...
    def __init__(
        ...
        client_id: str,
        max_retries: int = 20,  # Increased retries for slow Kafka startup
    ...
    def _get_topic_for_event(self, event_type: str) -> str:
        ...
        topic_mapping = {
            ...
            "user.email_verified": "user.events",
            "user.deactivated": "user.events",
            "user.reactivated": "user.events",
            "user.password_reset_requested": "user.events",
            "user.password_reset_confirmed": "user.events",
            "user.avatar_updated": "user.events",
            "user.address_created": "user.events",
            "user.address_updated": "user.events",
            "user.address_deleted": "user.events",
            "user.default_address_changed": "user.events",
            "profile.created": "user.events",
            "profile.updated": "user.events",
        }
        return topic_mapping.get(event_type, "user.events")
...
class KafkaEventSubscriber(EventSubscriber):
    """User Service Kafka subscriber with connection retry logic"""
