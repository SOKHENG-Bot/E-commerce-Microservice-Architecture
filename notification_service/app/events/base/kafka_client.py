...
class KafkaEventPublisher(EventPublisher):
    """
    Notification Service Kafka publisher with connection retry logic
    """
    ...
    async def ensure_topic_exists(self, topic_name: str):
        ...
        try:
            ...
            logger.warning(f"Error ensuring topic {topic_name} exists: {e}")
        finally:
    ...
    async def start(self, timeout: float = 30.0) -> None:
        ...
        async with self._connection_lock:
            ...
            for attempt in range(self.max_retries):
                try:
                    logger.info(
                        f"Attempting to connect to Kafka (attempt {attempt + 1}/{self.max_retries})"
                    )
    ...
    def _get_topic_for_event(self, event_type: str) -> str:
        ...
        topic_mapping = {
            ...
            "notification.delivered": "notification.events",
            "notification.failed": "notification.events",
            "notification.requested": "notification.events",
            "email.sent": "notification.events",
            "sms.sent": "notification.events",
            "push.sent": "notification.events",
        }
        return topic_mapping.get(event_type, "notification.events")
...
class KafkaEventSubscriber(EventSubscriber):
    """Notification Service Kafka subscriber with connection retry logic"""
    ...
    async def start(self, timeout: float = 30.0):
        ...
        for attempt in range(self.max_retries):
            try:
                logger.info(
                    f"Attempting to connect Kafka subscriber (attempt {attempt + 1}/{self.max_retries})"
                )
    ...
    async def subscribe(  # type: ignore
        ...
        if topic not in self.consumers:
            try:
                ...
                logger.info(f"Subscribed to {event_type} on topic {topic}")
