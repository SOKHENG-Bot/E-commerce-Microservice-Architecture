"""
Role-based authorization middleware for Order Service.
...
try:
    ...
    logger = setup_order_logging("order_service_roles")
except ImportError:
    logger = logging.getLogger("order_service_roles")
...
class OrderServiceRoleAuthorizationMiddleware(BaseHTTPMiddleware):
    """
    Role-based authorization middleware for Order Service.
    ...
    async def dispatch(
        ...
        try:
            ...
            user_roles_attr = getattr(request.state, "user_roles", None)
            user_roles: List[str] = (
                user_roles_attr if isinstance(user_roles_attr, list) else []
            )
...
def setup_order_role_authorization_middleware(
    app: FastAPI,
    role_requirements: Optional[Dict[str, Union[str, List[str]]]] = None,
    exclude_paths: Optional[List[str]] = None,
) -> None:
    """
    Setup role-based authorization middleware for Order Service.
    ...
    {
        "/api/v1/orders/admin": "admin",  # Admin only routes
        "/api/v1/orders/bulk": ["admin", "manager"],  # Admin or manager
        "/order-service": "admin",  # Management routes
    }
    """
    ...
    if role_requirements is None:
        # Default role requirements for order service patterns
        role_requirements = {
