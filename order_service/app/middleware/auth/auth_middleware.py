...
try:
    ...
    logger = setup_order_logging("order_service_auth")
except ImportError:
    logger = logging.getLogger("order_service_auth")
...
class OrderServiceAuthMiddleware(BaseHTTPMiddleware):
    """
    Authentication middleware for Order Service.
    ...
    def __init__(
        ...
        self.exclude_paths = exclude_paths or [
            ...
            "/redoc",
            "/openapi.json",
            "/api/v1/orders/health",  # Order service health endpoint
        ]
        ...
        if jwt_handler:
            ...
            from order_service.app.utils.jwt_handler import JWTHandler
    ...
    async def _validate_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            if not self.jwt_handler:
                ...
                logger.warning("JWT handler not available, using mock validation")
...
def setup_order_auth_middleware(
    app: FastAPI,
    exclude_paths: Optional[list[str]] = None,
    jwt_handler: Optional[Any] = None,
) -> None:
    """
    Setup authentication middleware for Order Service.
    ...
    app.add_middleware(
        OrderServiceAuthMiddleware, exclude_paths=exclude_paths, jwt_handler=jwt_handler
    )

    logger.info(
        "Order Service authentication middleware configured",
        extra={
            "service": "order_service",
